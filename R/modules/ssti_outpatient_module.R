# SSTI Outpatient Analysis Shiny Module - Complete Implementation (CORRECTED)
# This module contains all SSTI-specific analysis functionality split into separate tabs
# CORRECTED: Now uses ATC5 codes to match baseline R Markdown implementation

# Overview Tab UI
sstiOutpatientOverviewUI <- function(id) {
  ns <- NS(id)
  ...
  tagList(
    ...
    conditionalPanel(
      ...
      fluidRow(
        column(10, offset = 1,
               box(width = 12, title = "🎯 AWaRe Quality Indicators for Mild SSTI", status = "info", solidHeader = TRUE,
                   h5("Within the gPPS data structure, the following antibiotic use quality indicators have been identified:"),
                   p("1) Proportion of patients presenting with mild SSTI given an oral antibiotic."),
                   p("2) Proportion of patients presenting with mild SSTI given oral antibiotics by AWaRe category (Access or Watch)."),
                   p("3) Proportion of patients presenting with mild SSTI prescribed the total daily dose of oral antibiotics recommended in the WHO AWaRe Antibiotic Book."),
                   p("4) Proportion of patients presenting with mild SSTI given the duration in days of oral antibiotics recommended in the WHO AWaRe Antibiotic Book.")
               )
        )
      ),
...
sstiOutpatientOralUI <- function(id) {
  ns <- NS(id)
  ...
  tagList(
    ...
    conditionalPanel(
      ...
      fluidRow(
        column(10, offset = 1,
               box(width = 12, title = "📌 Indicator 1 - Oral Antibiotic Use", status = "primary", solidHeader = TRUE,
                   p("Proportion of patients presenting with mild SSTI given an oral antibiotic.")
               )
        )
      ),
      ...
      fluidRow(
        column(10, offset = 1,
               box(width = 12, title = "📈 Antibiotic Use by Route of Administration", status = "primary", solidHeader = TRUE,
                   p("This visual summarises the proportion of adult outpatients with mild SSTI given antibiotics by route of administration across specialties."),
      ...
      fluidRow(
        column(10, offset = 1,
               box(width = 12, title = "📝 Summary of Antibiotic Use for Mild SSTI by Route of Administration", status = "success", solidHeader = TRUE, collapsible = TRUE, collapsed = TRUE,
...
sstiOutpatientAWaReUI <- function(id) {
  ns <- NS(id)
  ...
  tagList(
    ...
    conditionalPanel(
      ...
      fluidRow(
        column(10, offset = 1,
               box(width = 12, title = "📌 Indicator 2 - Oral Antibiotic Use by AWaRe", status = "primary", solidHeader = TRUE,
                   p("Proportion of patients presenting with mild SSTI given oral antibiotics by AWaRe category (Access or Watch).")
               )
        )
      ),
      ...
      fluidRow(
        column(10, offset = 1,
               box(width = 12, title = "📈 Antibiotic Prescription by AWaRe Classification", status = "primary", solidHeader = TRUE,
                   p("This visual summarises the proportion of adult outpatients with mild SSTI given antibiotics by AWaRe classification across specialties."),
                   div(style = "display: flex; justify-content: center; align-items: center; max-width: 100%; overflow: hidden;",
      ...
      fluidRow(
        column(10, offset = 1,
               box(width = 12, title = "📝 Summary of Antibiotic Use for Mild SSTI by AWaRe Classification", status = "success", solidHeader = TRUE, collapsible = TRUE, collapsed = TRUE,
...
sstiOutpatientChoiceUI <- function(id) {
  ns <- NS(id)
  ...
  tagList(
    ...
    conditionalPanel(
      ...
      fluidRow(
        column(10, offset = 1,
               box(width = 12, title = "📌 Antibiotic Choice Alignment with AWaRe Book Recommendations for Mild SSTI", status = "primary", solidHeader = TRUE,
                   p("Proportion of patients presenting with mild SSTI prescribed the recommended oral antibiotic choice in the WHO AWaRe Antibiotic Book.")
               )
        )
      ),
      ...
      fluidRow(
        column(10, offset = 1,
               box(width = 12, title = "🔍 WHO AWaRe Book Recommendation:", status = "primary", solidHeader = TRUE,
                   tags$ul(
                     tags$li(strong("Amoxicillin+clavulanic acid"), " 500 mg+125 mg every 8 hours (ORAL) OR 1 g+200 mg every 8 hours (IV) for 5 days"),
                     tags$li(strong("Cefalexin"), " 500 mg every 8 hours (ORAL) for 5 days"),
                     tags$li(strong("Cloxacillin"), " 500 mg every 6 hours (ORAL) for 5 days")
                   )
               )
        )
      ),
      ...
      fluidRow(
        column(10, offset = 1,
               box(
                 ...
                 title = tagList(
                   icon("chart-bar"), 
                   " Antibiotic Choice Alignment Analysis for Mild SSTI"
                 ), 
                 status = "primary", 
                 solidHeader = TRUE,
                 tabsetPanel(
                   ...
                   tabPanel(
                     ...
                     div(
                       ...
                       br(),
                       p("This visual summarises the proportion of adult outpatients with mild SSTI prescribed the recommended oral antibiotic choice in the WHO AWaRe Antibiotic Book across specialties."),
                       div(
                         style = "
                           display: flex; 
                   ...
                   tabPanel(
                     ...
                     div(
                       style = "background-color: white; padding: 15px;",
                       br(),
                       p("This visual shows the distribution of recommended oral antibiotics for mild SSTI by WHO AWaRe classification."),
                       div(
                         style = "
                           display: flex; 
                           justify-content: center; 
      ...
      fluidRow(
        column(10, offset = 1,
               box(width = 12, title = "📝 Summary of Antibiotic Choice Appropriateness for Mild SSTI", status = "success", solidHeader = TRUE, collapsible = TRUE, collapsed = TRUE,
...
sstiOutpatientDosageUI <- function(id) {
  ns <- NS(id)
  ...
  tagList(
    ...
    conditionalPanel(
      ...
      fluidRow(
        column(10, offset = 1,
               box(width = 12, title = "📌 Indicator 3 – Oral Antibiotic Choice & Dosage Appropriateness", status = "primary", solidHeader = TRUE,
                   p("Proportion of patients presenting with mild SSTI prescribed the total daily dose of oral antibiotics recommended in the WHO AWaRe Antibiotic Book.")
               )
        )
      ),
      ...
      fluidRow(
        column(10, offset = 1,
               box(width = 12, title = "📈 Antibiotic Choice & Dosage Appropriateness for Mild SSTI", status = "primary", solidHeader = TRUE,
                   p("This visual summarises the proportion of adult outpatients with mild SSTI prescribed the total daily dose of oral antibiotics recommended in the WHO AWaRe Antibiotic Book across specialties."),
                   div(style = "display: flex; justify-content: center; align-items: center; max-width: 100%; overflow: hidden;",
      ...
      fluidRow(
        column(10, offset = 1,
               box(width = 12, title = "📝 Summary of Antibiotic Choice & Dosage Appropriateness for Mild SSTI", status = "success", solidHeader = TRUE, collapsible = TRUE, collapsed = TRUE,
...
sstiOutpatientDurationUI <- function(id) {
  ns <- NS(id)
  ...
  tagList(
    ...
    conditionalPanel(
      ...
      fluidRow(
        column(10, offset = 1,
               box(width = 12, title = "📌 Indicator 4 – Oral Antibiotic Duration Appropriateness", status = "primary", solidHeader = TRUE,
                   p("Proportion of patients presenting with mild SSTI given the duration in days of oral antibiotics recommended in the WHO AWaRe Antibiotic Book.")
               )
        )
      ),
      ...
      fluidRow(
        column(10, offset = 1,
               box(width = 12, title = "📈 Antibiotic Duration Appropriateness for Mild SSTI", status = "primary", solidHeader = TRUE,
                   p("This visual summarises the proportion of adult outpatients with mild SSTI given the duration in days of oral antibiotics recommended in the WHO AWaRe Antibiotic Book across specialties."),
                   div(style = "display: flex; justify-content: center; align-items: center; max-width: 100%; overflow: hidden;",
      ...
      fluidRow(
        column(10, offset = 1,
               box(width = 12, title = "📝 Summary of Antibiotic Duration Appropriateness for Mild SSTI", status = "success", solidHeader = TRUE, collapsible = TRUE, collapsed = TRUE,
...
sstiOutpatientAnalysisServer <- function(id, data_reactive) {
  moduleServer(id, function(input, output, session) {
    ...
    generate_eligibility_feedback <- function() {
      tryCatch({
        ...
        html_feedback <- HTML(paste0(
          ...
          "This script applies <strong>WHO AWaRe Quality Indicators</strong> to adult outpatients with mild SSTI.",
          "</p>",
          "<ul>",
          "<li><strong>gPPS recorded symptoms:</strong> Skin lesions/spots</li>",
    ...
    output$summary_insights_cards <- renderUI({
      ...
      # Calculate totals
      total_SSTI <- data_patients %>%
        filter(`Diagnosis code` == "SSTI") %>%
        distinct(`Unique Patient ID`) %>%
        nrow()
      
      total_SSTI_adults <- data_patients %>%
        filter(`Diagnosis code` == "SSTI", `Patient age group` == "ADULT") %>%
        distinct(`Unique Patient ID`) %>%
        nrow()
      
      total_SSTI_adults_abx <- data_patients %>%
        filter(`Diagnosis code` == "SSTI", `Patient age group` == "ADULT", AWaRe %in% AWaRe_abx) %>%
        distinct(`Unique Patient ID`) %>%
        nrow()
      
      total_SSTI_adults_prescriptions <- data_patients %>%
        filter(`Diagnosis code` == "SSTI", `Patient age group` == "ADULT", AWaRe %in% AWaRe_abx) %>%
        nrow()
      ...
      insight_cards <- HTML(glue::glue(
        ...
        <div style='flex: 1; min-width: 250px; background-color: #e6f4ea; border-left: 6px solid #009E73; padding: 12px; border-radius: 5px;'>
          <strong>Eligible Patients with Mild SSTI (Adults):</strong><br>
          <span style='font-size: 1.3em;'>{total_SSTI_adults}</span><br>
          <small>{round(100 * total_SSTI_adults / total_SSTI, 1)}% (of {total_SSTI} patients with mild SSTI)</small>
        </div>
        ...
        <div style='flex: 1; min-width: 250px; background-color: #fff3cd; border-left: 6px solid #D55E00; padding: 12px; border-radius: 5px;'>
          ...
          <span style='font-size: 1.3em;'>{round(100 * total_SSTI_adults_abx / total_SSTI_adults, 1)}%</span><br>
          <small>({total_SSTI_adults_abx} of {total_SSTI_adults} eligible patients)</small>
        </div>
        ...
        <div style='flex: 1; min-width: 250px; background-color: #f0f8ff; border-left: 6px solid #0072B2; padding: 12px; border-radius: 5px;'>
          ...
          <span style='font-size: 1.3em;'>{total_SSTI_adults_prescriptions}</span>
        </div>
    ...
    output$patient_level_table <- DT::renderDataTable({
      ...
      patient_summary <- data.frame(
        Category = c(
          "Number of all patients with a diagnosis of mild SSTI",
          "Number of adult patients (≥18 years) with mild SSTI",
          "Number of eligible patients: Adult patients with mild SSTI who received antibiotics"
        ),
        Count = c(
          total_patients,
          data_patients %>%
    ...
    output$prescription_level_table <- DT::renderDataTable({
      ...
      prescription_summary <- data.frame(
        Category = c(
          "Number of all antibiotic prescriptions for patients diagnosed with mild SSTI",
          "Number of antibiotic prescriptions for adult patients with mild SSTI",
          "Number of eligible antibiotic prescriptions"
        ),
        Count = c(
          total_prescriptions,
          data_patients %>% filter(`Diagnosis code` == "SSTI", `Patient age group` == "ADULT") %>% nrow(),
          data_patients %>% filter(`Diagnosis code` == "SSTI", `Patient age group` == "ADULT", AWaRe %in% AWaRe_abx) %>% nrow()
        )
      ) %>%
        mutate(
    ...
    output$oral_summary <- renderUI({
      ...
      data_SSTI <- oral_data$data_SSTI
      
      # Denominator
      total_eligible <- data_SSTI %>%
        filter(`Patient age group` == "ADULT") %>%
        distinct(`Unique Patient ID`) %>%
        nrow()
      
      total_eligible_with_abx <- data_SSTI %>%
      ...
      intro_text <- glue::glue(
        ...
        💊 <strong>Denominator:</strong> Number of eligible patients who received any antibiotic for mild SSTI (<strong>{total_eligible_with_abx}</strong> out of {total_eligible}).
        </div><br><br>
    ...
    # Oral plot - with hover text, NO GRAY HIGHLIGHT
    output$oral_plot <- renderPlotly({
      ...
      oral_summary_long <- oral_summary_long %>%
        ...
        mutate(
          ...
          hover_text = paste0(
            ...
            "<b>Count:</b> ", Count, "<br>",
            "<b>Total:</b> ", Total, "<br>",
            "<b>Proportion:</b> ", scales::percent(Proportion, accuracy = 0.1)
          )
        ) %>%
        ungroup()
      ...
      # Order labels (Facility-Wide first)
      label_order <- c("<b style='color:#0072B2;'>Facility-Wide</b>",
                       sort(setdiff(unique(oral_summary_long$PlotLabel), "<b style='color:#0072B2;'>Facility-Wide</b>")))
      oral_summary_long$PlotLabel <- factor(oral_summary_long$PlotLabel, levels = label_order)
      
      # Colors (keep original SSTI colors)
      custom_palette <- c(
        "Oral" = "#0072B2",
        "IV/IM" = "#3399ff",
        "Others" = "#6F42C1"
      ...
      p <- ggplot(
        oral_summary_long,
        aes(
          x = PlotLabel, y = Proportion, fill = Antibiotic_Type,
          text = hover_text
        )
      ) +
        ...
        geom_text(
          data = oral_summary_long %>% distinct(PlotLabel, Total),
      ...
      # Convert to plotly - NO GRAY HIGHLIGHT
      plt <- ggplotly(p, tooltip = "text") %>%
        layout(
          title = list(
            text = paste0(
              "<b>Proportion of Patients with Mild SSTI by Antibiotic Route</b><br>"
            ),
    ...
    output$aware_summary <- renderUI({
      ...
      data_SSTI <- aware_data$data_SSTI
      
      # Denominator
      total_eligible2 <- data_SSTI %>%
        filter(`Patient age group` == "ADULT") %>%
        distinct(`Unique Patient ID`) %>%
        nrow()
      
      total_eligible_with_abx2 <- data_SSTI %>%
      ...
      intro_text2 <- glue::glue(
        "<div style='background-color: #f8f9fa; border-left: 5px solid #17a2b8; padding: 14px; margin-top: 10px; margin-bottom: 10px;'>
        💊 <strong>Denominator:</strong> Number of eligible patients who received any oral antibiotic for mild SSTI (<strong>{total_eligible_with_abx2}</strong> out of {total_eligible2}).
        </div><br><br>
        <strong>Summary:</strong><br><br>"
      )
    ...
    # AWaRe plot for SSTI - Modified with hover text, NO GRAY HIGHLIGHT
    output$aware_plot <- renderPlotly({
      ...
      # Define stacking and legend order
      ...
      aware_plot_data$AWaRe_Cat <- factor(aware_plot_data$AWaRe_Cat, levels = aware_levels_stack)
      
      # Format labels and add hover text
      aware_plot_data <- aware_plot_data %>%
        mutate(
          ...
          hover_text = paste0(
            ...
            "<b>AWaRe Category:</b> ", as.character(AWaRe_Cat), "<br>",
            "<b>Count:</b> ", Count, "<br>",
      ...
      ordered_labels <- c(
        "<b style='color:#0072B2;'>Facility-Wide</b>",
        sort(unique(aware_plot_data$PlotLabel[aware_plot_data$PlotLabel != "<b style='color:#0072B2;'>Facility-Wide</b>"]))
      )
      aware_plot_data$PlotLabel <- factor(aware_plot_data$PlotLabel, levels = ordered_labels)
      ...
      # Plot
      p <- ggplot(aware_plot_data, aes(y = PlotLabel, x = ProportionPct, fill = AWaRe_Cat, text = hover_text)) +
      ...
      plt <- ggplotly(p, tooltip = "text") %>%
        layout(
          title = list(
            text = paste0(
              "<b>Distribution of Oral Antibiotic Use in Mild SSTI by AWaRe Category</b><br>"
            ),
            x = 0.5, xanchor = "center",
            y = 0.98, yanchor = "top",
            font = list(size = 11)
    ...
    # Choice data reactive - CORRECTED TO USE ATC5
    choice_data_reactive <- reactive({
      ...
      # Prepare the data - CORRECTED: uses ATC5 instead of Antimicrobial name
      data_SSTI_choice <- data_patients %>%
        filter(`Diagnosis code` == "SSTI") %>%
        mutate(
          Route = toupper(Route),
          ATC5 = trimws(toupper(ATC5)),  # CORRECTED: Use ATC5
      ...
      # Lookup ATC codes for SSTI QI - CORRECTED: uses ABX-ATC
      lookup_names <- data_lookup %>%
        filter(Code == "PC_SSTI_APPROP_DOSAGE_ORAL_ABX") %>%
        select(starts_with("ABX-ATC")) %>%  # CORRECTED: Use ABX-ATC
        unlist(use.names = FALSE)
      
      # Flag matched prescriptions - CORRECTED: uses ATC5
      data_SSTI_choice <- data_SSTI_choice %>%
        mutate(
          Drug_Match = ATC5 %in% lookup_names  # CORRECTED: Use ATC5
        )
      
      # Extract lookup info
      lookup_SSTI <- data_lookup %>%
        filter(Code == "PC_SSTI_APPROP_DOSAGE_ORAL_ABX")
      
      # Create long format from lookup - CORRECTED: uses ABX-ATC
      lookup_long <- tibble(
        Drug = unlist(lookup_SSTI %>% select(starts_with("ABX-ATC")), use.names = FALSE),  # CORRECTED
        Choice = unlist(lookup_SSTI %>% select(starts_with("ABX-CHOICE")), use.names = FALSE)
      ) %>%
        filter(!is.na(Drug))
      
      # Merge choice info with patient-level data - CORRECTED: uses ATC5
      data_SSTI_choice <- data_SSTI_choice %>%
        left_join(lookup_long, by = c("ATC5" = "Drug"))  # CORRECTED
      
      # Patient Summary - CORRECTED: uses ATC5
      patient_summary <- data_SSTI_choice %>%
        filter(AWaRe_compatible) %>%
        group_by(`Unique Patient ID`, `Type of specialty`) %>%
        summarise(
          All_ABX = list(unique(ATC5)),  # CORRECTED
          
          # Match flags (3 drugs)
          Match_1 = any(ATC5 == lookup_names[1]),  # CORRECTED
          Match_2 = any(ATC5 == lookup_names[2]),  # CORRECTED
          Match_3 = any(ATC5 == lookup_names[3]),  # CORRECTED
          
          Match_1_O = any(ATC5 == lookup_names[1] & Route == "O"),  # CORRECTED
          Match_2_O = any(ATC5 == lookup_names[2] & Route == "O"),  # CORRECTED
          Match_3_O = any(ATC5 == lookup_names[3] & Route == "O"),  # CORRECTED
          
          N_ABX = n_distinct(ATC5),  # CORRECTED
          Any_O = any(Route == "O"),
          .groups = "drop"
        ) %>%
        rowwise() %>%
        mutate(
          Num_Recommended_O = sum(c_across(Match_1_O:Match_3_O)),
      ...
      no_abx_patients <- data_SSTI_choice %>%
        group_by(`Unique Patient ID`, `Type of specialty`) %>%
        summarise(
      ...
      not_eligible_patients <- data_SSTI_choice %>%
        group_by(`Unique Patient ID`, `Type of specialty`) %>%
      ...
      # Combine
      qi_summary_SSTI <- bind_rows(qi_summary_SSTI, hospital_data)
      
      # Recalculate totals
      qi_summary_SSTI <- qi_summary_SSTI %>%
        group_by(`Type of specialty`) %>%
        mutate(
          Total = sum(Patients),
    ...
    # Choice plot (main alignment) - with hover text, NO GRAY HIGHLIGHT
    output$choice_plot <- renderPlotly({
      ...
      qi_summary_SSTI <- choice_data$qi_summary_SSTI
      ...
      # Create styled labels with hover text
      qi_summary_SSTI <- qi_summary_SSTI %>%
        mutate(
          PlotLabel = ifelse(
            `Type of specialty` == "Facility-Wide",
      ...
      # Plot
      p <- ggplot(qi_summary_SSTI, aes(x = PlotLabel, y = Proportion, fill = Indicator, text = hover_text)) +
        geom_bar(stat = "identity", position = "fill", width = 0.85) +
        geom_text(
        ...
        geom_text(
          data = qi_summary_SSTI %>% distinct(PlotLabel, Total),
          aes(x = PlotLabel, y = 1.05, label = paste0("n=", Total)),
          inherit.aes = FALSE,
          size = 3, color = "gray30", hjust = 0.6
        ) +
        scale_fill_manual(values = drug_choice_colors) +
      ...
      plt <- ggplotly(p, tooltip = "text") %>%
        layout(
          title = list(
            text = "<b>Antibiotic Choice Alignment for Mild SSTI</b>",
            x = 0.5,
            xanchor = "center",
            y = 0.97,
            yanchor = "top",
    ...
    # Choice AWaRe plot - CORRECTED TO USE ATC5
    output$choice_aware_plot <- renderPlotly({
      ...
      data_SSTI_choice <- choice_data$data_SSTI_choice
      patient_summary <- choice_data$patient_summary
      
      data <- data_reactive()
      data_lookup <- data$data_lookup
      
      # Lookup names (3 drugs) - CORRECTED: uses ABX-ATC
      lookup_names <- data_lookup %>%
        filter(Code == "PC_SSTI_APPROP_DOSAGE_ORAL_ABX") %>%
        select(starts_with("ABX-ATC")) %>%  # CORRECTED
        unlist(use.names = FALSE)
      
      # Patient summary by AWaRe - CORRECTED: uses ATC5
      patient_summary_AWARE <- data_SSTI_choice %>%
        filter(`Patient age group` == "ADULT", AWaRe %in% AWaRe_abx) %>%
        group_by(`Unique Patient ID`, `Type of specialty`, AWaRe) %>%
        summarise(
          N_total = n(),
          N_match = sum(ATC5 %in% lookup_names),  # CORRECTED
          N_O = sum(Route == "O"),
          N_O_match = sum(Route == "O" & ATC5 %in% lookup_names),  # CORRECTED
    ...
    output$choice_summary <- renderUI({
      ...
      relevant_data <- qi_summary_SSTI_filtered %>%
        filter(Indicator %in% c(
          "Received recommended oral antibiotics",
          "Partially received recommended oral antibiotics",
          "Received oral antibiotics not among recommended options"
      ...
      if (nrow(relevant_data) == 0) {
        return(HTML(
          ...
          ⚠️ <strong>No summary available</strong> — there are currently no eligible patients for assessing oral antibiotic choice appropriateness for mild SSTI.<br><br>
      ...
      # Summarise data
      summary_data_SSTI <- relevant_data %>%
        group_by(`Type of specialty`, Indicator) %>%
        summarise(Patients = sum(Patients), .groups = "drop") %>%
        pivot_wider(
      ...
      hospital_row <- summary_data_SSTI %>%
        summarise(
          `Type of specialty` = "Facility-Wide",
      ...
      # Combine
      summary_data_combined <- bind_rows(hospital_row, summary_data_SSTI)
      
      # Intro
      intro_text <- glue::glue(
        "<div style='background-color: #f8f9fa; border-left: 5px solid #17a2b8; padding: 14px; margin-top: 10px; margin-bottom: 10px;'>
        💊 <strong>Denominator:</strong> Number of eligible patients who received any oral antibiotics for mild SSTI (<strong>{total_iv}</strong> out of {total_eligible}).
        </div><br><br>
        <strong>Summary:</strong><br><br>"
      )
    ...
    # Dosage data reactive - CORRECTED TO USE ATC5
    dosage_data_reactive <- reactive({
      ...
      # Filter and prepare - CORRECTED: uses ATC5
      data_SSTI2 <- data_patients %>%
        filter(`Diagnosis code` == "SSTI") %>%
        mutate(
          Route = toupper(Route),
          ATC5 = trimws(toupper(ATC5)),  # CORRECTED: Use ATC5
      ...
      # Lookup (3 drugs) - CORRECTED: uses ABX-ATC
      lookup2 <- data_lookup %>% filter(Code == "PC_SSTI_APPROP_DOSAGE_ORAL_ABX")
      lookup_names2 <- unlist(lookup2[1, c("ABX-ATC-1", "ABX-ATC-2", "ABX-ATC-3")], use.names = FALSE)  # CORRECTED
      lookup_names2 <- toupper(trimws(lookup_names2))
      
      # Compute Total Daily Dose
      data_SSTI2 <- data_SSTI2 %>%
        mutate(
      ...
      # Match Drug + Dose + Route (3 drugs) - CORRECTED: uses ATC5
      ...
      # Perform matching (3 drugs) - CORRECTED: uses ATC5
      data_SSTI2 <- data_SSTI2 %>%
        mutate(
          Match_Drug_Dose_1 = ATC5 == drug_names[1] & Route == "O" &  # CORRECTED
            abs(Total_Daily_Dose - expected_doses[1]) < 1,
          
          Match_Drug_Dose_2 = ATC5 == drug_names[2] & Route == "O" &  # CORRECTED
            abs(Total_Daily_Dose - expected_doses[2]) < 1,
          
          Match_Drug_Dose_3 = ATC5 == drug_names[3] & Route == "O" &  # CORRECTED
            abs(Total_Daily_Dose - expected_doses[3]) < 1
        )
      
      # Patient summary (3 drugs) - CORRECTED: uses ATC5
      patient_summary2 <- data_SSTI2 %>%
        filter(AWaRe_compatible) %>%
        group_by(`Unique Patient ID`, `Type of specialty`) %>%
        summarise(
          Match_1_O = any(ATC5 == lookup_names2[1] & Route == "O"),  # CORRECTED
          Match_2_O = any(ATC5 == lookup_names2[2] & Route == "O"),  # CORRECTED
          Match_3_O = any(ATC5 == lookup_names2[3] & Route == "O"),  # CORRECTED
    ...
    # Dosage plot - with hover text, NO GRAY HIGHLIGHT
    output$dosage_plot <- renderPlotly({
      ...
      final_summary2 <- final_summary2 %>%
        mutate(
          ...
          # Calculate proportion for plotting (0-1 scale)
          Plot_Proportion = Patients / Total,
          hover_text = paste0(
            "<b>Specialty:</b> ", gsub("<.*?>", "", as.character(PlotLabel)), "<br>",
      ...
      # Plot
      p <- ggplot(final_summary2, aes(x = PlotLabel, y = Plot_Proportion, fill = Dose_Result, text = hover_text)) +
        geom_bar(stat = "identity", position = "fill", width = 0.85) +
        geom_text(
          aes(label = ifelse(Plot_Proportion > 0.05, paste0(round(Proportion, 1), "%"), "")),
          position = position_fill(vjust = 0.5),
      ...
      plt <- ggplotly(p, tooltip = "text") %>%
        layout(
          title = list(
            text = "<b>Antibiotic Choice & Dosage for Mild SSTI</b>",
            x = 0.5,
            xanchor = "center",
            y = 0.98,
            yanchor = "top",
    ...
    output$dosage_summary <- renderUI({
      ...
      final_summary2 <- dosage_data$final_summary2
      data_SSTI2 <- dosage_data$data_SSTI2
      
      # Filter
      final_summary2_SSTI_filtered <- final_summary2 %>%
        filter(`Type of specialty` != "Facility-Wide")
      ...
      total_approp_iv_given <- final_summary2_SSTI_filtered %>%
        filter(Dose_Result %in% dose_categories) %>%
        summarise(Total = sum(Patients, na.rm = TRUE)) %>%
        pull(Total)
      
      relevant_dose_data <- final_summary2_SSTI_filtered %>%
        filter(Dose_Result %in% dose_categories)
    ...
    <strong>Summary:</strong><br><br>"
      ...
      if (nrow(complete_summary) == 0) {
        return(HTML(
          "<div style='background-color: #fff3cd; border-left: 5px solid #ffc107; padding: 14px; margin-top: 10px;'>
      ⚠️ <strong>No summary available</strong> — there are currently no eligible patients for evaluating the <strong>appropriateness of oral antibiotic dosage</strong> for mild SSTI.<br><br>
      <em>This may indicate that no patients met the inclusion criteria, or no patients received recommended oral antibiotics during the reporting period.</em>
      </div>"
        ))
      }
      ...
      formatted_blocks2 <- complete_summary %>%
        ...
        summarise(
          block = {
            ...
            cat1_prop <- Proportion[Dose_Result == dose_categories[1]]
            cat1_n <- Patients[Dose_Result == dose_categories[1]]
            cat2_prop <- Proportion[Dose_Result == dose_categories[2]]
            cat2_n <- Patients[Dose_Result == dose_categories[2]]
            total_n <- first(Total)
          ...
          <strong>🏥 {dept}</strong> <span style='color: #888;'>(n = {total_n} patients)</span><br><br>
          <ul style='margin-left: 1.2em; line-height: 1.7; padding-left: 0; list-style-type: none;'>
            <li>✅ <strong>Received recommended oral antibiotic with recommended dosage</strong> (as per WHO AWaRe Book): <strong>{scales::percent(cat1_prop, accuracy = 0.1)}</strong> ({cat1_n} out of {total_n})</li>
            <li>❌ <strong>Received recommended oral antibiotic without recommended dosage</strong> (as per WHO AWaRe Book): <strong>{scales::percent(cat2_prop, accuracy = 0.1)}</strong> ({cat2_n} out of {total_n})</li>
          </ul>
          </div>"
            )
          },
    ...
    # Duration data reactive - CORRECTED TO USE ATC5
    duration_data_reactive <- reactive({
      ...
      # Extract durations - CORRECTED: uses ABX-ATC
      lookup_duration <- tibble(
        Drug = toupper(trimws(unlist(lookup2 %>% select(starts_with("ABX-ATC"))))),  # CORRECTED
      ...
      # Filter data - CORRECTED: uses ATC5
      data_SSTI4 <- data_patients %>%
        filter(`Diagnosis code` == "SSTI") %>%
        mutate(
          Route = toupper(Route),
          ATC5 = trimws(toupper(ATC5)),  # CORRECTED
          AWaRe_compatible = ifelse(
            `Patient age group` == "ADULT" & AWaRe %in% AWaRe_abx,
            TRUE, FALSE
          )
      ...
      # Merge duration info - CORRECTED: uses ATC5
      data_SSTI4 <- data_SSTI4 %>%
        left_join(lookup_duration, by = c("ATC5" = "Drug"))  # CORRECTED
    ...
    # Duration plot - with hover text, NO GRAY HIGHLIGHT
    output$duration_plot <- renderPlotly({
      ...
      plt <- ggplotly(p, tooltip = "text") %>%
        layout(
          title = list(
            text = "<b>Duration Appropriateness for Mild SSTI</b>",
            x = 0.5,
            xanchor = "center",
            y = 0.98,
            yanchor = "top",
    ...
    output$duration_summary <- renderUI({
      ...
      duration_viz_data <- duration_data$duration_viz_data
      data_SSTI4 <- duration_data$data_SSTI4
