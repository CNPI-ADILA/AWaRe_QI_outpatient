# RTI Infections Analysis Shiny Module - Complete Implementation
# This module contains all RTI infection-specific analysis functionality split into separate tabs

# Overview Tab UI
rtiOverviewUI <- function(id) {
  ns <- NS(id)
  ...
  tagList(
    ...
    conditionalPanel(
      ...
      fluidRow(
        column(10, offset = 1,
               box(width = 12, title = "🎯 AWaRe Quality Indicators for RTI Infections", status = "info", solidHeader = TRUE,
                   h5("Within the gPPS data structure, the following antibiotic use quality indicators have been identified:"),
                   p("1) Proportion of patients presenting with RIT infections given oral Amoxicillin or other oral antibiotics."),
                   p("2) Proportion of patients presenting with RTI infections given oral antibiotics by AWaRe category (Access or Watch)."),
                   p("3) Proportion of patients presenting with RTI infections prescribed the total daily dose of oral antibiotics recommended in the WHO AWaRe Antibiotic Book."),
                   p("4) Proportion of patients presenting with RTI infections given the duration in days of oral antibiotics recommended in the WHO AWaRe Antibiotic Book.")
               )
        )
      )
    )
...
rtiOralUI <- function(id) {
  ...
  tagList(
    ...
    conditionalPanel(
      ...
      fluidRow(
        column(10, offset = 1,
               box(width = 12, title = "📌 Indicator 1 - Oral Antibiotic Use", status = "primary", solidHeader = TRUE,
                   p("Proportion of patients presenting with RIT infections given oral Amoxicillin or other oral antibiotics.")
               )
        )
      ),
      ...
      fluidRow(
        column(10, offset = 1,
               box(width = 12, title = "📈 Antibiotic Use by Route of Administration", status = "primary", solidHeader = TRUE,
                   p("This visual summarises the proportion of adult outpatients with RTI infections given antibiotics by route of administration across specialties."),
      ...
      fluidRow(
        column(10, offset = 1,
               box(width = 12, title = "📝 Summary of Antibiotic Use for RTI Infections by Route of Administration", status = "success", solidHeader = TRUE, collapsible = TRUE, collapsed = TRUE,
...
rtiAWaReUI <- function(id) {
  ...
  tagList(
    ...
    conditionalPanel(
      ...
      fluidRow(
        column(10, offset = 1,
               box(width = 12, title = "📌 Indicator 2 - Oral Antibiotic Use by AWaRe", status = "primary", solidHeader = TRUE,
                   p("Proportion of patients presenting with RTI infections given oral antibiotics by AWaRe category (Access or Watch).")
               )
        )
      ),
      ...
      fluidRow(
        column(10, offset = 1,
               box(width = 12, title = "📈 Antibiotic Prescription by AWaRe Classification", status = "primary", solidHeader = TRUE,
                   p("This visual summarises the proportion of adult outpatients with RTI infections given antibiotics by AWaRe classification across specialties."),
                   div(style = "display: flex; justify-content: center; align-items: center; max-width: 100%; overflow: hidden;",
      ...
      fluidRow(
        column(10, offset = 1,
               box(width = 12, title = "📝 Summary of Antibiotic Use for RTI Infections by AWaRe Classification", status = "success", solidHeader = TRUE, collapsible = TRUE, collapsed = TRUE,
...
rtiChoiceUI <- function(id) {
  ...
  tagList(
    ...
    conditionalPanel(
      ...
      fluidRow(
        column(10, offset = 1,
               box(width = 12, title = "📌 Antibiotic Choice Alignment with AWaRe Book Recommendations for RTI Infections", status = "primary", solidHeader = TRUE,
                   p("Proportion of patients presenting with RTI infections prescribed the recommended oral antibiotic choice in the WHO AWaRe Antibiotic Book.")
               )
        )
      ),
      ...
      fluidRow(
        column(10, offset = 1,
               box(width = 12, title = "🔍 WHO AWaRe Book Recommendation:", status = "primary", solidHeader = TRUE,
                   tags$p(strong("For Mild and Moderate CAP:")),
                   tags$ul(
                     tags$li(strong("Amoxicillin"), " 1 g every 8 hours (ORAL)"),
                     tags$li(strong("Phenoxymethylpenicillin"), " 500 mg (800,000 IU) every 6 hours (ORAL)"),
                     tags$li(strong("Amoxicillin + Clavulanic acid"), " 875 mg + 125 mg every 8 hours (ORAL)"),
                     tags$li(strong("Doxycycline"), " 100 mg every 12 hours (ORAL)")
                   ),
                   tags$p(strong("For Severe CAP:")),
                   tags$ul(
                     tags$li(strong("Cefotaxime"), " 2 g every 8 hours (IV/IM)"),
                     tags$li(strong("Ceftriaxone"), " 2 g every 24 hours (IV) or 1 g every 24 hours (IM*)"),
                     tags$li(strong("Amoxicillin + Clavulanic acid"), " 1 g + 200 mg every 8 hours (IV)"),
                     tags$li("If CURB-65 ≥ 2: Consider adding: ", strong("Clarithromycin"), " 500 mg every 12 hours (ORAL or IV)")
                   ),
                   tags$p(strong("For acute otitis media:")),
                   tags$ul(
                     tags$li(strong("Amoxicillin"), " 500 mg every 8 hours (ORAL)"),
                     tags$li(strong("Amoxicillin+clavulanic acid"), " 500 mg+125 mg every 8 hours (ORAL)")
                   ),
                   tags$p(strong("For Acute sinusitis:")),
                   tags$ul(
                     tags$li(strong("Amoxicillin"), " 1 g every 8 hours (ORAL)"),
                     tags$li(strong("Amoxicillin+clavulanic acid"), " 500 mg+125 mg every 8 hours (ORAL)")
                   ),
                   tags$p(strong("For Pharyngitis:")),
                   tags$ul(
                     tags$li(strong("Amoxicillin"), " 1 g every 8 hours (ORAL)"),
                     tags$li(strong("Phenoxymethylpenicillin"), " 500 mg (800 000 IU) every 6 hours (ORAL)"),
                     tags$li(strong("Cefalexin"), " 500 mg every 8 hours (ORAL)"),
                     tags$li(strong("Clarithromycin"), " 500 mg every 12 hours (ORAL)")
                   ),
                   tags$p(strong("Duration:")),
                   tags$ul(
                     tags$li("Treat for 5 days. If severe disease, consider longer treatment and look for complications such as empyema, if patient not clinically stable at day 5")
                   )
               )
        )
      ),
      ...
      fluidRow(
        column(10, offset = 1,
               box(
                 ...
                 title = tagList(
                   icon("chart-bar"), 
                   " Antibiotic Choice Alignment Analysis for RTI Infections"
                 ), 
                 status = "primary", 
                 solidHeader = TRUE,
                 tabsetPanel(
                   ...
                   tabPanel(
                     ...
                     div(
                       ...
                       br(),
                       p("This visual summarises the proportion of adult outpatients with RTI infections prescribed the recommended oral antibiotic choice in the WHO AWaRe Antibiotic Book across specialties."),
                       div(
                         style = "
                           display: flex; 
                   ...
                   tabPanel(
                     ...
                     div(
                       style = "background-color: white; padding: 15px;",
                       br(),
                       p("This visual shows the distribution of recommended oral antibiotics for RTI infections by WHO AWaRe classification."),
                       div(
                         style = "
                           display: flex; 
                           justify-content: center; 
                   ...
                   # Tab 3: Line of Treatment
                   tabPanel(
                     tagList(icon("list-ol"), " Line of Treatment"),
                     div(
                       style = "background-color: white; padding: 15px;",
                       br(),
                       p("This visual shows the distribution of recommended oral antibiotics for RTI infections by line of treatment (First choice, Second choice)."),
                       div(
                         style = "
                           display: flex; 
                           justify-content: center; 
      ...
      fluidRow(
        column(10, offset = 1,
               box(width = 12, title = "📝 Summary of Antibiotic Choice Appropriateness for RTI Infections", status = "success", solidHeader = TRUE, collapsible = TRUE, collapsed = TRUE,
...
rtiDosageUI <- function(id) {
  ...
  tagList(
    ...
    conditionalPanel(
      ...
      fluidRow(
        column(10, offset = 1,
               box(width = 12, title = "📌 Indicator 3 – Oral Antibiotic Dosage Appropriateness", status = "primary", solidHeader = TRUE,
                   p("Proportion of patients presenting with RTI infections prescribed the total daily dose of oral antibiotics recommended in the WHO AWaRe Antibiotic Book.")
               )
        )
      ),
      ...
      fluidRow(
        column(10, offset = 1,
               box(width = 12, title = "📈 Antibiotic Dosage Appropriateness for RTI Infections", status = "primary", solidHeader = TRUE,
                   p("This visual summarises the proportion of adult outpatients with RTI infections prescribed the total daily dose of oral antibiotics recommended in the WHO AWaRe Antibiotic Book across specialties."),
                   div(style = "display: flex; justify-content: center; align-items: center; max-width: 100%; overflow: hidden;",
      ...
      fluidRow(
        column(10, offset = 1,
               box(width = 12, title = "📝 Summary of Antibiotic Dosage Appropriateness for RTI Infections", status = "success", solidHeader = TRUE, collapsible = TRUE, collapsed = TRUE,
...
rtiDurationUI <- function(id) {
  ...
  tagList(
    ...
    conditionalPanel(
      ...
      fluidRow(
        column(10, offset = 1,
               box(width = 12, title = "📌 Indicator 4 – Oral Antibiotic Duration Appropriateness", status = "primary", solidHeader = TRUE,
                   p("Proportion of patients presenting with RTI infections given the duration in days of oral antibiotics recommended in the WHO AWaRe Antibiotic Book.")
               )
        )
      ),
      ...
      fluidRow(
        column(10, offset = 1,
               box(width = 12, title = "📈 Antibiotic Duration Appropriateness for RTI Infections", status = "primary", solidHeader = TRUE,
                   p("This visual summarises the proportion of adult outpatients with RTI infections given the duration in days of oral antibiotics recommended in the WHO AWaRe Antibiotic Book across specialties."),
                   div(style = "display: flex; justify-content: center; align-items: center; max-width: 100%; overflow: hidden;",
      ...
      fluidRow(
        column(10, offset = 1,
               box(width = 12, title = "📝 Summary of Antibiotic Duration Appropriateness for RTI Infections", status = "success", solidHeader = TRUE, collapsible = TRUE, collapsed = TRUE,
...
# Module Server - Handles all RTI infection tabs
rtiAnalysisServer <- function(id, data_reactive) {
  moduleServer(id, function(input, output, session) {
    ...
    generate_eligibility_feedback <- function() {
      tryCatch({
        ...
        # Filter eligible RTI infection patients
        data_rti <- data_patients %>%
          filter(`Diagnosis code` == "RTI") %>%
          mutate(
            Route = toupper(as.character(Route)),
            AWaRe_compatible = (`Patient age group` == "ADULT")
          ) 
        
        # Count eligible unique RTI cases
        eligible_rti_n <- data_rti %>%
          filter(AWaRe_compatible) %>%
          distinct(`Unique Patient ID`) %>%
          nrow()
        
        # Build status message
        status_message <- if(eligible_rti_n == 0) {
        ...
        html_feedback <- HTML(paste0(
          ...
          "This script applies <strong>WHO AWaRe Quality Indicators</strong> to adult outpatients with RTI infections.",
          "</p>",
          "<ul>",
          "<li><strong>Total eligible cases:</strong> ", eligible_rti_n, "</li>",
          "</ul>",
          status_message,
          "</div>"
        ))
    ...
    output$summary_insights_cards <- renderUI({
      ...
      # Calculate totals - EXACTLY as in baseline
      total_rti <- data_patients %>%
        filter(`Diagnosis code` == "RTI") %>%
        distinct(`Unique Patient ID`) %>%
        nrow()
      
      total_rti_adults <- data_patients %>%
        filter(`Diagnosis code` == "RTI", `Patient age group` == "ADULT") %>%
        distinct(`Unique Patient ID`) %>%
        nrow()
      
      total_rti_adults_abx <- data_patients %>%
        filter(`Diagnosis code` == "RTI", `Patient age group` == "ADULT", AWaRe %in% AWaRe_abx) %>%
        distinct(`Unique Patient ID`) %>%
        nrow()
      
      # This counts ALL prescription ROWS, not unique patients - matches baseline exactly
      total_rti_adults_prescriptions <- data_patients %>%
        filter(`Diagnosis code` == "RTI", `Patient age group` == "ADULT", AWaRe %in% AWaRe_abx) %>%
        nrow()
      ...
      insight_cards <- HTML(glue::glue(
        ...
        <div style='flex: 1; min-width: 250px; background-color: #e6f4ea; border-left: 6px solid #009E73; padding: 12px; border-radius: 5px;'>
          <strong>Eligible Patients with RTI Infections (Adults):</strong><br>
          <span style='font-size: 1.3em;'>{total_rti_adults}</span><br>
          <small>{round(100 * total_rti_adults / total_rti, 1)}% (of {total_rti} patients with RTI infections)</small>
        </div>
        ...
        <div style='flex: 1; min-width: 250px; background-color: #fff3cd; border-left: 6px solid #D55E00; padding: 12px; border-radius: 5px;'>
          ...
          <span style='font-size: 1.3em;'>{round(100 * total_rti_adults_abx / total_rti_adults, 1)}%</span><br>
          <small>({total_rti_adults_abx} of {total_rti_adults} eligible patients)</small>
        </div>
        ...
        <div style='flex: 1; min-width: 250px; background-color: #f0f8ff; border-left: 6px solid #0072B2; padding: 12px; border-radius: 5px;'>
          ...
          <span style='font-size: 1.3em;'>{total_rti_adults_prescriptions}</span>
        </div>
    ...
    # Oral antibiotic use data - WITH AMOXICILLIN SPLIT (as in baseline)
    oral_data_reactive <- reactive({
      ...
      # Pre-clean data - matches baseline exactly
      data_rti <- data_patients %>%
        filter(`Diagnosis code` == "RTI", `Patient age group` == "ADULT") %>%
        mutate(
          Route = toupper(Route),
      ...
      # Patient-level summary - WITH AMOXICILLIN SPLIT
      rti_oral_summary <- data_rti %>%
        group_by(`Unique Patient ID`, `Type of specialty`) %>%
        summarise(
          Oral_Amox = any(Route == "O" & ATC5 == "J01CA04"),  # AMOXICILLIN
          Oral_Other = any(Route == "O" & ATC5 != "J01CA04"),
      ...
      oral_summary_by_spec <- rti_oral_summary %>%
        ...
        summarise(
          Patients = n(),
          Oral_Amox = sum(Oral_Amox),
          Oral_Other = sum(Oral_Other),
          IV_IM_Abx = sum(Any_IV_IM),
          Other_Abx = sum(Any_Other),
          Pct_Oral_Amox = round(100 * Oral_Amox / Patients, 1),
          Pct_Oral_Other = round(100 * Oral_Other / Patients, 1),
          Pct_IV_Abx = round(100 * IV_IM_Abx / Patients, 1),
    ...
    output$oral_summary <- renderUI({
      ...
      data_rti <- oral_data$data_rti
      
      # Denominator - matches baseline
      total_eligible <- data_rti %>%
        filter(`Patient age group` == "ADULT") %>%
        distinct(`Unique Patient ID`) %>%
        nrow()
      
      total_eligible_with_abx <- data_rti %>%
      ...
      intro_text <- glue::glue(
        ...
        💊 <strong>Denominator:</strong> Number of eligible patients who received any antibiotic for RTI (<strong>{total_eligible_with_abx}</strong> out of {total_eligible}).
        </div><br><br>
        <strong>Summary:</strong><br><br>"
      )
      
      # Format blocks - WITH AMOXICILLIN
      oral_blocks <- oral_summary_final %>%
        mutate(
          block = pmap_chr(list(
            `Type of specialty`, Patients,
            Oral_Amox, Pct_Oral_Amox,
            Oral_Other, Pct_Oral_Other,
            IV_IM_Abx, Pct_IV_Abx,
            Other_Abx, Pct_Other_Abx
          ), function(dept, n, amox_n, amox_pct, oral_n, oral_pct, iv_n, iv_pct, other_n, other_pct) {
            ...
            glue::glue(
              ...
              <ul style='margin-left: 1.2em; line-height: 1.8; padding-left: 0; list-style-type: none;'>
                <li>
                  <span style='display: inline-block; background-color: #28a745; color: white; border-radius: 50%; width: 22px; height: 22px; text-align: center; line-height: 22px; font-size: 12px; margin-right: 8px;'>1</span>
                  <strong>Given oral amoxicillin:</strong> {amox_pct}% ({amox_n} of {n})
                </li>
                <li>
                  <span style='display: inline-block; background-color: #007bff; color: white; border-radius: 50%; width: 22px; height: 22px; text-align: center; line-height: 22px; font-size: 12px; margin-right: 8px;'>2</span>
                  <strong>Given any other oral antibiotic:</strong> {oral_pct}% ({oral_n} of {n})
                </li>
                <li>
                  <span style='display: inline-block; background-color: #dc3545; color: white; border-radius: 50%; width: 22px; height: 22px; text-align: center; line-height: 22px; font-size: 12px; margin-right: 8px;'>3</span>
                  <strong>Given any IV/IM antibiotic:</strong> {iv_pct}% ({iv_n} of {n})
                </li>
                <li>
                  <span style='display: inline-block; background-color: #6f42c1; color: white; border-radius: 50%; width: 22px; height: 22px; text-align: center; line-height: 22px; font-size: 12px; margin-right: 8px;'>4</span>
    ...
    # Oral plot - WITH AMOXICILLIN SEPARATELY
    output$oral_plot <- renderPlotly({
      ...
      oral_summary_long <- oral_summary_final %>%
        select(`Type of specialty`, Patients, Oral_Amox, Oral_Other, IV_IM_Abx, Other_Abx) %>%
        pivot_longer(
          cols = c(Oral_Amox, Oral_Other, IV_IM_Abx, Other_Abx),
          names_to = "Antibiotic_Type",
          values_to = "Count"
        ) %>%
        mutate(
          Antibiotic_Type = recode(Antibiotic_Type,
                                   Oral_Amox = "Oral Amoxicillin",
                                   Oral_Other = "Other oral",
                                   IV_IM_Abx = "IV/IM",
                                   Other_Abx = "Others"
          ),
          Percent = round(100 * Count / Patients, 1),
          Antibiotic_Type = factor(Antibiotic_Type, levels = c("Oral Amoxicillin", "Other oral", "IV/IM", "Others")),
          hover_text = paste0(
      ...
      # Colors - WITH AMOXICILLIN
      custom_palette <- c(
        "Oral Amoxicillin" = "#009E73",  # Green
        "Other oral" = "#0072B2",        # Blue
        "IV/IM" = "#3399ff",             # Orange/red
      ...
      plt <- ggplotly(p, tooltip = "text") %>%
        layout(
          title = list(
            text = "<b>Proportion of Patients with RTI by Antibiotic Route and Type</b>",
            x = 0.5,
            xanchor = "center",
            y = 0.98,
            yanchor = "top",
    ...
    # AWaRe data reactive - matches baseline exactly
    aware_data_reactive <- reactive({
      ...
      # Widen data - matches baseline exactly
      aware_summary <- data_rti %>%
    ...
    # AWaRe summary - matches baseline
    output$aware_summary <- renderUI({
      ...
      data_rti <- aware_data$data_rti
      
      # Denominator
      total_eligible2 <- data_rti %>%
        filter(`Patient age group` == "ADULT") %>%
        distinct(`Unique Patient ID`) %>%
        nrow()
      
      total_eligible_with_abx2 <- data_rti %>%
      ...
      intro_text2 <- glue::glue(
        "<div style='background-color: #f8f9fa; border-left: 5px solid #17a2b8; padding: 14px; margin-top: 10px; margin-bottom: 10px;'>
        💊 <strong>Denominator:</strong> Number of eligible patients who received any oral antibiotic for RTI (<strong>{total_eligible_with_abx2}</strong> out of {total_eligible2}).
        </div><br><br>
        <strong>Summary:</strong><br><br>"
      )
    ...
    output$aware_plot <- renderPlotly({
      ...
      plt <- ggplotly(p, tooltip = "text") %>%
        layout(
          title = list(
            text = "<b>Distribution of Oral Antibiotic Use in RTI by AWaRe Category</b>",
            x = 0.5,
            xanchor = "center",
            y = 0.98,
            yanchor = "top",
    ...
    # Choice data reactive - Uses ATC5 as in baseline
    choice_data_reactive <- reactive({
      ...
      # Prepare the data - uses ATC5
      data_rti_choice <- data_patients %>%
        filter(`Diagnosis code` == "RTI") %>%
        mutate(
          Route = toupper(Route),
          ATC5 = toupper(trimws(ATC5)),
      ...
      # Lookup ABX ATC5 codes for RTI QI
      lookup_names <- data_lookup %>%
        filter(Code == "PC_RTI_APPROP_DOSAGE_ORAL_ABX") %>%
        select(starts_with("ABX-ATC")) %>%
        unlist(use.names = FALSE)
      
      lookup_names <- toupper(trimws(lookup_names))
      
      # Flag matched prescriptions
      data_rti_choice <- data_rti_choice %>%
        mutate(
          Drug_Match = ATC5 %in% lookup_names
        )
      
      # Extract lookup info
      lookup_rti <- data_lookup %>%
        filter(Code == "PC_RTI_APPROP_DOSAGE_ORAL_ABX")
      
      # Create long format from lookup
      lookup_long <- tibble(
        Drug = toupper(trimws(unlist(lookup_rti %>% select(starts_with("ABX-ATC")), use.names = FALSE))),
        Choice = trimws(unlist(lookup_rti %>% select(starts_with("ABX-CHOICE")), use.names = FALSE))
      ) %>%
        filter(!is.na(Drug))
      ...
      # Patient Summary - matches baseline exactly
      patient_summary <- data_rti_choice %>%
        filter(AWaRe_compatible) %>%
        group_by(`Unique Patient ID`, `Type of specialty`) %>%
        summarise(
          All_ABX = list(unique(ATC5)),
          
          # Match flags for up to 6 drugs
        ...
        rowwise() %>%
        mutate(
          Num_Recommended_O = sum(c_across(Match_1_O:Match_6_O)),
      ...
      no_abx_patients <- data_rti_choice %>%
        group_by(`Unique Patient ID`, `Type of specialty`) %>%
        summarise(
      ...
      not_eligible_patients <- data_rti_choice %>%
        group_by(`Unique Patient ID`, `Type of specialty`) %>%
      ...
      # Combine
      qi_summary_rti <- bind_rows(qi_summary_rti, hospital_data)
      
      # Recalculate totals
      qi_summary_rti <- qi_summary_rti %>%
        group_by(`Type of specialty`) %>%
        mutate(
          Total = sum(Patients),
    ...
    # Choice plot (main alignment) - matches baseline
    output$choice_plot <- renderPlotly({
      ...
      qi_summary_rti <- choice_data$qi_summary_rti
      ...
      # Plot
      p <- ggplot(qi_summary_rti, aes(x = PlotLabel, y = Proportion, fill = Indicator, text = hover_text)) +
        ...
        geom_text(
          data = qi_summary_rti %>% distinct(PlotLabel, Total),
      ...
      plt <- ggplotly(p, tooltip = "text") %>%
        layout(
          title = list(
            text = "<b>Antibiotic Choice Alignment for RTI</b>",
    ...
    # Choice AWaRe plot - matches baseline
    output$choice_aware_plot <- renderPlotly({
      ...
      data_rti_choice <- choice_data$data_rti_choice
      ...
      patient_summary_AWARE <- data_rti_choice %>%
        filter(`Patient age group` == "ADULT", AWaRe %in% AWaRe_abx) %>%
        ...
        mutate(
          Appropriate_oral = (Match_1_O | Match_2_O | Match_3_O | Match_4_O | Match_5_O | Match_6_O)
        )
    ...
    # Choice Line of Treatment plot - matches baseline
    output$choice_line_plot <- renderPlotly({
      ...
      choice_summary <- data_rti_choice %>%
        filter(`Patient age group` == "ADULT", AWaRe %in% AWaRe_abx, Route == "O", !is.na(Choice)) %>%
        group_by(`Type of specialty`, Choice) %>%
      ...
      # Add Facility-Wide as a summary row
      hospital_summary <- data_rti_choice %>%
        filter(`Patient age group` == "ADULT", AWaRe %in% AWaRe_abx, Route == "O", !is.na(Choice)) %>%
        group_by(Choice) %>%
        summarise(Prescriptions = n(), .groups = "drop") %>%
        mutate(`Type of specialty` = "Facility-Wide") %>%
      ...
      all_combos <- expand_grid(
        `Type of specialty` = unique(choice_summary$`Type of specialty`),
        Choice = choice_levels_stack
      )
      
      choice_summary <- all_combos %>%
        left_join(choice_summary, by = c("Type of specialty", "Choice")) %>%
        mutate(
      ...
      choice_summary <- choice_summary %>%
        mutate(
          PlotLabel = ifelse(`Type of specialty` == "Facility-Wide",
                             "<b style='color:#0072B2;'>Facility-Wide</b>",
                             `Type of specialty`),
          hover_text = paste0(
            "<b>Specialty:</b> ", gsub("<.*?>", "", as.character(PlotLabel)), "<br>",
            "<b>Line:</b> ", Choice, "<br>",
            "<b>Prescriptions:</b> ", Prescriptions, "<br>",
            "<b>Total:</b> ", Total, "<br>",
            "<b>Proportion:</b> ", round(Proportion * 100, 1), "%"
          )
        )
      
      # Factor the PlotLabel in reversed order (Facility-Wide on top)
      ordered_labels <- c(
        "<b style='color:#0072B2;'>Facility-Wide</b>",
        sort(unique(choice_summary$PlotLabel[choice_summary$PlotLabel != "<b style='color:#0072B2;'>Facility-Wide</b>"]))
      )
      choice_summary$PlotLabel <- factor(choice_summary$PlotLabel, levels = rev(ordered_labels))
      
      # Separate label data for geom_text (n=)
      label_data <- data_rti_choice %>%
        filter(`Patient age group` == "ADULT", AWaRe %in% AWaRe_abx, Route == "O", !is.na(Choice)) %>%
        count(`Type of specialty`, name = "Total")
      
      # Add Facility-Wide total manually
      ...
      # Create plot
      p <- ggplot(choice_summary, aes(x = PlotLabel, y = Proportion, fill = Choice, text = hover_text)) +
        geom_bar(stat = "identity", position = "fill", width = 0.85) +
        geom_text(
          aes(label = ifelse(Proportion > 0.05, scales::percent(Proportion, accuracy = 1), "")),
          position = position_fill(vjust = 0.5),
      ...
      plt <- ggplotly(p, tooltip = "text") %>%
        layout(
          title = list(
            text = "<b>Recommended Oral Antibiotics by Line of Treatment</b>",
            x = 0.5,
            xanchor = "center",
            y = 0.98,
            yanchor = "top",
    ...
    # Choice summary - matches baseline
    output$choice_summary <- renderUI({
      ...
      relevant_data <- qi_summary_rti_filtered %>%
        filter(Indicator %in% c(
          "Received recommended oral antibiotics",
          "Partially received recommended oral antibiotics",
          "Received oral antibiotics not among recommended options"
      ...
      if (nrow(relevant_data) == 0) {
        return(HTML(
          ...
          ⚠️ <strong>No summary available</strong> — there are currently no eligible patients for assessing oral antibiotic choice appropriateness for RTI.<br><br>
      ...
      # Summarise data
      summary_data_rti <- relevant_data %>%
        group_by(`Type of specialty`, Indicator) %>%
        summarise(Patients = sum(Patients), .groups = "drop") %>%
        pivot_wider(
      ...
      # Combine
      summary_data_combined <- bind_rows(hospital_row, summary_data_rti)
      
      # Intro
      intro_text <- glue::glue(
        "<div style='background-color: #f8f9fa; border-left: 5px solid #17a2b8; padding: 14px; margin-top: 10px; margin-bottom: 10px;'>
        💊 <strong>Denominator:</strong> Number of eligible patients who received any oral antibiotics for RTI (<strong>{total_oral}</strong> out of {total_eligible}).
        </div><br><br>
        <strong>Summary:</strong><br><br>"
      )
    ...
    # Dosage data reactive - Uses ATC5 and MU conversion as in baseline
    dosage_data_reactive <- reactive({
      ...
      # Lookup
      lookup2 <- data_lookup %>% filter(Code == "PC_RTI_APPROP_DOSAGE_ORAL_ABX")
      ...
      # Compute Total Daily Dose with MU → IU conversion (EXACTLY as in baseline)
      data_rti2 <- data_rti2 %>%
        mutate(
          Unit_Factor = case_when(
            ...
            Unit == "g"  ~ 1000,
            Unit == "IU" ~ 1,
            Unit == "MU" ~ 1e6,  # MU to IU conversion
            TRUE ~ NA_real_
          ),
          Total_Daily_Dose = as.numeric(`Single unit dose`) * as.numeric(`N Doses/day`) * Unit_Factor
        )
      
      # Match Drug + Dose + Route (EXACTLY as in baseline, including alternative doses)
      ...
      # Special logic for Drugs with alternative doses (EXACTLY as in baseline)
      expected_1_main <- expected_doses[1]
      expected_1_alt <- as.numeric(lookup2[["ABX-DOSE-1a"]][1]) *
        as.numeric(lookup2[["ABX-DAY-DOSE-1"]][1]) *
        convert_unit(lookup2[["ABX-UNIT-1a"]][1])
      
      expected_2_main <- expected_doses[2]
      expected_2_alt <- as.numeric(lookup2[["ABX-DOSE-2a"]][1]) *
        as.numeric(lookup2[["ABX-DAY-DOSE-2"]][1]) *
        convert_unit(lookup2[["ABX-UNIT-2a"]][1])
      
      expected_3_main <- expected_doses[3]
      expected_3_alt <- as.numeric(lookup2[["ABX-DOSE-3a"]][1]) *
        as.numeric(lookup2[["ABX-DAY-DOSE-3"]][1]) *
        convert_unit(lookup2[["ABX-UNIT-3"]][1])
      
      # Perform matching
      data_rti2 <- data_rti2 %>%
        mutate(
          Match_Drug_Dose_1 = ATC5 == drug_names[1] & Route == "O" &
            (abs(Total_Daily_Dose - expected_1_main) < 1 | abs(Total_Daily_Dose - expected_1_alt) < 1),
          ...
          Match_Drug_Dose_3 = ATC5 == drug_names[3] & Route == "O" &
            (abs(Total_Daily_Dose - expected_3_main) < 1 | abs(Total_Daily_Dose - expected_3_alt) < 1),
      ...
      patient_summary2 <- data_rti2 %>%
        ...
        mutate(
          Any_Match = Match_1_O | Match_2_O | Match_3_O | Match_4_O | Match_5_O | Match_6_O,
          Any_Correct_Dose = 
            (Match_1_O & Dose_1_OK) |
            (Match_2_O & Dose_2_OK) |
            (Match_3_O & Dose_3_OK) |
      ...
      # Summary by specialty
      oral_dose_counts <- patient_summary2 %>%
      ...
      oral_dose_summary2 <- all_combos2 %>%
        left_join(oral_dose_counts, by = c("Type of specialty", "Dose_Result")) %>%
        mutate(Patients = replace_na(Patients, 0)) %>%
        group_by(`Type of specialty`) %>%
    ...
    output$dosage_plot <- renderPlotly({
      ...
      plt <- ggplotly(p, tooltip = "text") %>%
        layout(
          title = list(
            text = "<b>Antibiotic Choice & Dosage for RTI</b>",
            x = 0.5,
            xanchor = "center",
            y = 0.98,
            yanchor = "top",
    ...
    # Dosage summary - matches baseline
    output$dosage_summary <- renderUI({
      ...
      final_summary2 <- dosage_data$final_summary2
      data_rti2 <- dosage_data$data_rti2
      
      # Filter
      final_summary2_rti_filtered <- final_summary2 %>%
        filter(`Type of specialty` != "Facility-Wide")
      ...
      total_approp_oral_given <- final_summary2_rti_filtered %>%
        filter(Dose_Result %in% dose_categories) %>%
        summarise(Total = sum(Patients, na.rm = TRUE)) %>%
        pull(Total)
      
      relevant_dose_data <- final_summary2_rti_filtered %>%
        filter(Dose_Result %in% dose_categories)
      ...
      intro_text2 <- glue::glue(
        "<div style='background-color: #f8f9fa; border-left: 5px solid #17a2b8; padding: 14px; margin-top: 10px; margin-bottom: 10px;'>
    💊 <strong>Denominator:</strong> Number of eligible patients who received any recommended oral antibiotic choice based on WHO AWaRe Book (<strong>{total_approp_oral_given}</strong> out of {total_eligible}).
    </div><br><br>
    <strong>Summary:</strong><br><br>"
      )
      ...
      if (nrow(complete_summary) == 0) {
        return(HTML(
          "<div style='background-color: #fff3cd; border-left: 5px solid #ffc107; padding: 14px; margin-top: 10px;'>
      ⚠️ <strong>No summary available</strong> — there are currently no eligible patients for evaluating the <strong>appropriateness of oral antibiotic dosage</strong> for RTI.<br><br>
      <em>This may indicate that no patients met the inclusion criteria, or no patients received recommended oral antibiotics during the reporting period.</em>
      </div>"
        ))
      }
    ...
    # Duration data reactive - Uses ATC5 as in baseline
    duration_data_reactive <- reactive({
      ...
      # Extract durations - matches baseline exactly
    ...
    # Duration plot - matches baseline
    output$duration_plot <- renderPlotly({
      ...
      plt <- ggplotly(p, tooltip = "text") %>%
        layout(
          title = list(
            text = "<b>Duration Appropriateness for RTI</b>",
            x = 0.5,
            xanchor = "center",
            y = 0.98,
            yanchor = "top",
    ...
    # Duration summary - matches baseline
    output$duration_summary <- renderUI({
      ...
      duration_viz_data <- duration_data$duration_viz_data
      data_rti4 <- duration_data$data_rti4
      ...
      # Calculate Facility-Wide total (denominator numerator)
      ...
      # Total eligible adult RTI patients (denominator total)
      total_eligible <- data_rti4 %>%
        filter(`Patient age group` == "ADULT") %>%
        distinct(`Unique Patient ID`) %>%
        nrow()
      ...
      intro_text <- glue::glue(
        "<div style='background-color: #f8f9fa; border-left: 5px solid #17a2b8; padding: 14px; margin-top: 10px; margin-bottom: 10px;'>
        💊 <strong>Denominator:</strong> Number of eligible patients who received recommended oral antibiotic choice (based on WHO AWaRe Book) 
        (<strong>{hospital_total_patients}</strong> out of {total_eligible}).
        </div><br><br>
        <strong>Summary:</strong><br><br>"
      )
